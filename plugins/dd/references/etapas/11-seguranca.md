...
# na cadencia em vez de repeti-los aqui. osv-scanner e semgrep ainda não
# têm check formal no núcleo:
osv-scanner --lockfile=package-lock.json --json > .protocolo/$STAMP/11-osv.json
semgrep --config=p/owasp-top-ten --config=p/react --config=p/nextjs --config=p/typescript --json > .protocolo/$STAMP/11-semgrep.json
```

**Reportar:** achados por id de check (severidade, `bloqueia` sim/não), mapa OWASP Top 10 preenchido, findings do `osv-scanner` (OSV/GHSA) e do `semgrep` por severidade, SBOM (gerado pelo check `DEP-005`), guidance React/Next.js sem achado pendente.
